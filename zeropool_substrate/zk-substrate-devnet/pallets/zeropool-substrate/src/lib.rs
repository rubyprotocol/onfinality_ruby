...
pub mod pallet {
    ...
    #[pallet::storage]
    pub type VerificationKey<T: Config> = StorageMap<_, Twox64Concat, T::AccountId, (Vec<u8>, BalanceOf<T>), OptionQuery>;

    #[pallet::event]
    #[pallet::generate_deposit(pub(super) fn deposit_event)]
    pub enum Event<T: Config> {
		VerificationKeySet(T::AccountId),
		VerificationKeyUpdated(T::AccountId),
		VerificationSuccessful(T::AccountId),
		VerificationFailed(T::AccountId),
    }
    #[pallet::error]
    pub enum Error<T> {
		TooShort,
    ...
    impl<T: Config> Pallet<T> {
		...
		pub fn set_vk(origin: OriginFor<T>, vkb: Vec<u8>) -> DispatchResult {
			...
			let deposit = if let Some((_, deposit)) = <VerificationKey<T>>::get(&sender) {
				Self::deposit_event(Event::VerificationKeyUpdated(sender.clone()));
				deposit
			} else {
				let deposit = T::ReservationFee::get();
				T::Currency::reserve(&sender, deposit)?;
				Self::deposit_event(Event::VerificationKeySet(sender.clone()));
				deposit
			};
		...
		#[pallet::weight(500_000)]
		pub fn test_groth16verify(origin: OriginFor<T>, jproofinput: Vec<u8>) -> DispatchResult {
			let sender = ensure_signed(origin)?; //check is signed
			...
			match r {
				Ok(rs) => proofinput = rs,
				Err(_e) => {
								Self::deposit_event(Event::VerificationFailed(sender));
			...
			if groth16verify(vkd,proofd,inputd){
				// Return a successful DispatchResult
				Self::deposit_event(Event::VerificationSuccessful(sender));
				Ok(())
			}
			else{
...
fn groth16verify(vkd: VK, proofd: Proof, inputd: VU256) -> bool {
	...
	let pairing_expr =
		vec![(neg_a, proofd.b), (vkd.alpha, vkd.beta), (acc, vkd.gamma), (proofd.c, vkd.delta)];
