...
async fn returng_h() -> Result<impl warp::Reply, warp::Rejection> {
    ...
    let g =
        EdwardsPoint::<Bn256Fr>::rand(&mut rng, &jubjub_params).mul(Num::from(8), &jubjub_params);
...
async fn decrypt_wrapperqua(quaitem: QuaInputItem) -> Result<impl warp::Reply, warp::Rejection> {
    ...
    let sgp_cipher = SgpCipher { g1_mul_gamma, a, b };
    ...
    let f: BigNumMatrix = BigNumMatrix::new_bigints(
        &data,
        quaitem.dks.n_rows,
        quaitem.dks.n_cols,
        &BigNum::fromstring(m0),
    );
    let sgp_dk = SgpDecKey { key, f };
